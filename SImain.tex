...
Inspired by ~\cite{Mahadevan2019,Wolde2019}, in this 3D model, we model a fluid vesicle using a closed surface consisting of particles that are triangulated together. The energy of the system is $E=\sum (k_A(A-A_0)^2 + k_\theta (\theta-\pi/3)^2 + k_\phi (\phi-\pi)^2$), here A is the area that three particles make, $\theta$ is the angle they make and $\phi$ is the angle that two adjacent planes make. In this model, the number of bonds between particles is limited from 5 to 7 because particles with bonds not on this range are very energy unfavorable. Similar to the 2D model, we attempt to add and remove particle in our Monte Carlo simulation. In the addition move, a point is chosen in a triangulated surface to add the new particle.  The number of bonds this particle will have is randomly chosen from 5 to 7. For this to happen, some of the neighboring bonds will be destroyed to make room for the new ones. Thus, in this model, the connectivity between the particles is not conserved. In the removal move, a particle will be fused to one of its neighbors. Like the 2D model simulation, the acceptance rate for addition is $\rm{Min}(\rm{Exp}[\beta(-\Delta E +\mu]),1)$ and the removal rate is $\rm{Min}(\rm{Exp}[\beta(-\Delta E -\mu)],1)$.
